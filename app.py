...
def generate_interpretation(name, gender, oheng, survey, bmi):
    saju_intro = "사주(四柱)는 사람이 태어난 연도, 월, 일, 시를 기준으로 하는 네 개의 기둥을 의미하며, 각 기둥은 천간과 지지로 구성되어 있습니다. 이를 통해 총 여덟 글자로 이루어진 팔자(八字)를 구성하게 됩니다.
"
    saju_intro += "이 사주팔자를 바탕으로 개인의 타고난 기질, 성격, 인생 흐름, 건강 상태 등을 분석할 수 있습니다.
"
    saju_intro += "특히 건강 측면에서는 사주의 오행 구성과 균형이 중요한 역할을 하며, 어떤 오행이 강하거나 약한지를 통해 질병의 경향성을 유추할 수 있습니다.
"
    saju_intro += "사주팔자는 단순한 운세를 넘어서, 동양의학과 접목되어 개인 맞춤형 건강 관리에도 응용되고 있습니다.
"
    saju_intro += "
""{name}({gender})님의 사주는 '{oheng}' 오행이 주로 나타납니다.
"
    saju_intro += f"오행은 동양 철학에서 만물을 구성하는 다섯 가지 기본 요소로, 사람의 체질과 성향에도 깊은 영향을 미친다고 여겨집니다.
"
    saju_intro += f"'{oheng}' 오행은 신체 장기 중 특정 부분과 기능을 상징하며, 사람의 성격, 에너지 흐름, 건강 상태에 밀접하게 연관됩니다.
"
    saju_intro += f"예를 들어 '목(木)'은 간과 근육, '화(火)'는 심장과 혈액순환, '토(土)'는 위장과 소화기관, '금(金)'은 폐와 면역, '수(水)'는 신장과 뇌와 연관됩니다.
"
    saju_intro += f"또한, 특정 오행이 강하거나 약한 경우, 성향이나 질병 발생 경향이 나타날 수 있습니다.
"
    saju_intro += f"'{oheng}'의 특성은 다음과 같습니다:
"
...
"
    ...
    saju_intro += "오행의 균형은 사람마다 다르며, 특정 오행이 부족하거나 과할 경우 건강에 영향을 줄 수 있습니다.
"
    saju_intro += "이에 따라 영양제도 오행의 기운을 보완하는 방향으로 추천됩니다.
"
    saju_intro += "이제 건강 설문과 신체 정보(BMI 등)를 바탕으로 건강 분석을 진행하겠습니다.
...
if st.button("분석 및 추천하기"):
    ...
    st.subheader("📊 건강 분석 결과")
    explanation = generate_interpretation(name, gender, saju_oheng, survey, bmi)
